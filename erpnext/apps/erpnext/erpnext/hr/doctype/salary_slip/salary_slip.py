...
class SalarySlip(TransactionBase):
	...
	def get_data_for_eval(self):
		...
		data.update(frappe.get_doc("Salary Structure Assignment",
			{"customer": self.customer, "salary_structure": self.salary_structure}).as_dict())

		data.update(frappe.get_doc("Employee", self.employee).as_dict())
		data.update(frappe.get_doc("Customer",self.customer).as_dict())
	...
	def check_sal_struct(self, joining_date, relieving_date):
		cond = """and sa.customer=%(customer)s and sa.designation=%(designation)s and (sa.from_date <= %(start_date)s or
		...
		st_name = frappe.db.sql("""
			...
			limit 1
		""" %cond, {'customer': self.customer,'designation':self.designation, 'start_date': self.start_date,
